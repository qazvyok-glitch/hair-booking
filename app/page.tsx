...
function PriceSheet({ onClose, dark }: { onClose: () => void; dark: boolean }) {
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 100, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
      <div style={{ width: "100%", maxWidth: 390, background: bg, borderRadius: "20px 20px 0 0", maxHeight: "85vh", overflowY: "auto", padding: "20px 16px 32px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 600, color: text }}>價目表</div>
            <div style={{ fontSize: 11, color: sub, marginTop: 2 }}>染、燙、護髮依髮長及髮量收費</div>
          </div>
        ...
        {priceData.map((cat) => (
          <div key={cat.category} style={{ marginBottom: 18 }}>
            <div style={{ fontSize: 14, fontWeight: 600, borderBottom: "1px solid " + (dark ? "#3A3A3A" : "#D3D1C7"), paddingBottom: 6, marginBottom: 8, color: text }}>{cat.category}</div>
            {cat.items.map((item) => (
              <div key={item.name} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "0.5px solid " + (dark ? "#333" : "#F1EFE8") }}>
                <div style={{ fontSize: 12, color: text, flex: 1 }}>{item.name}</div>
                <div style={{ fontSize: 12, color: "#7B6FD4", fontWeight: 500, marginLeft: 8 }}>{item.range}</div>
              </div>
            ))}
          </div>
        ))}
        <div style={{ textAlign: "center", fontSize: 11, color: sub }}>消費滿 $2000 以上，歡迎使用 LINE PAY</div>
      </div>
    </div>
  );
}
...
export default function Home() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: bg, display: "flex", justifyContent: "center", padding: "24px 16px" }}>
      {showPrice && <PriceSheet onClose={() => setShowPrice(false)} dark={dark} />}
      <div style={{ width: "100%", maxWidth: 390, background: cardBg, borderRadius: 20, overflow: "hidden", border: "1px solid " + cardBorder }}>
        ...
        <div style={{ background: cardBg, padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "0.5px solid " + cardBorder }}>
          <span style={{ fontSize: 16, fontWeight: 500, color: textMain }}>Bing Cherry Hair Salon</span>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <button onClick={() => setDark(!dark)} style={{ background: dark ? "#333" : "#F1EFE8", border: "none", borderRadius: 20, padding: "5px 10px", fontSize: 13, cursor: "pointer", color: dark ? "#F0EFEA" : "#5F5E5A" }}>
              {dark ? "☀" : "☾"}
            </button>
        ...
        <div style={{ padding: 16 }}>
          <input placeholder="搜尋服務、設計師、風格..." style={{ width: "100%", background: inputBg, border: "none", borderRadius: 10, padding: "9px 14px", fontSize: 13, color: textSub, marginBottom: 12, outline: "none" }} />

          {/* 設計師 */}
          <div style={{ fontSize: 11, color: textSub, marginBottom: 7 }}>選擇設計師</div>
          <div style={{ display: "flex", gap: 8, marginBottom: 14, overflowX: "auto", paddingBottom: 4 }}>
            {designers.map((d) => (
              <div key={d.id} onClick={() => { setSelectedDesigner(d); setSelectedDate(null); setSelectedTime(null); }} style={{ flexShrink: 0, width: 100, background: selectedDesigner.id === d.id ? (dark ? "#2C2840" : "#EEEDFE") : cardBg, border: `0.5px solid ${selectedDesigner.id === d.id ? "#534AB7" : cardBorder}`, borderRadius: 12, padding: "10px 6px", textAlign: "center", cursor: "pointer" }}>
                ...
                <div style={{ fontSize: 12, fontWeight: 500, color: textMain }}>{d.name}</div>
                {d.nickname ? <div style={{ fontSize: 10, color: "#7B6FD4", margin: "2px 0" }}>{d.nickname}</div> : null}
                <div style={{ fontSize: 10, color: textSub, margin: "2px 0", lineHeight: 1.4 }}>{d.style}</div>
                <a href={"https://www.instagram.com/" + d.ig} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} style={{ display: "inline-flex", alignItems: "center", gap: 3, marginTop: 5, textDecoration: "none" }}>
                  <IgIcon />
                  <span style={{ fontSize: 10, color: textSub }}>作品集</span>
                </a>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            ...
            <button onClick={() => setShowPrice(true)} style={{ fontSize: 11, color: "#7B6FD4", background: dark ? "#2C2840" : "#EEEDFE", border: "none", borderRadius: 8, padding: "4px 10px", cursor: "pointer" }}>查看價目表 →</button>
          </div>
            ...
            return (
              <div key={cat.id} style={{ width: "100%", marginBottom: 4 }}>
                <div onClick={() => setOpenCategory(isOpen ? null : cat.id)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: pickedCount > 0 ? (dark ? "#2C2840" : cat.color) : cardBg, border: `0.5px solid ${pickedCount > 0 ? cat.textColor + "40" : cardBorder}`, borderRadius: isOpen ? "10px 10px 0 0" : 10, cursor: "pointer" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 13, fontWeight: 500, color: textMain }}>{cat.label}</span>
                  ...
                  <span style={{ fontSize: 11, color: textSub }}>{isOpen ? "▲" : "▼"}</span>
                </div>
                {isOpen && (
                    ...
                    {cat.items.map((item) => {
                      ...
                      return (
                        <div key={item.id} onClick={() => toggleService(item.id)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", background: picked ? (dark ? "#2C2840" : cat.color) : cardBg, borderTop: "0.5px solid " + (dark ? "#333" : "#F1EFE8"), cursor: "pointer" }}>
                          <div>
                            <div style={{ fontSize: 12, fontWeight: picked ? 500 : 400, color: textMain }}>{item.name}</div>
                            <div style={{ fontSize: 10, color: textSub, marginTop: 2 }}>{item.duration}</div>
                          </div>
          ...
          <div style={{ marginTop: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 500, color: textMain }}>{year} 年 {month} 月</span>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 3, marginBottom: 4 }}>
              {weekDays.map((d) => <div key={d} style={{ textAlign: "center", fontSize: 10, color: textSub, padding: "2px 0" }}>{d}</div>)}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 3, marginBottom: 12 }}>
              {calendarCells.map((day, idx) => {
                ...
                return (
                  <div key={idx} onClick={() => { if (!disabled) { setSelectedDate(dateStr); setSelectedTime(null); } }} style={{ textAlign: "center", padding: "6px 0", borderRadius: 8, fontSize: 12, cursor: disabled ? "default" : "pointer", background: isSelected ? "#534AB7" : isOff ? (dark ? "#222" : "#F1EFE8") : isToday ? (dark ? "#2C2840" : "#EEEDFE") : cardBg, color: isSelected ? "#fff" : disabled ? (dark ? "#444" : "#C8C6BE") : isSunday ? "#E24B4A" : textMain, border: `0.5px solid ${isSelected ? "#534AB7" : isToday ? "#AFA9EC" : cardBorder}`, fontWeight: isToday ? 600 : 400 }}>
                    {day}
                    {isOff && !isPast && <div style={{ fontSize: 7, color: dark ? "#555" : "#B4B2A9", lineHeight: 1 }}>休</div>}
                  </div>
                );
              })}
            </div>
          ...
          {selectedDate && (
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 11, color: textSub, marginBottom: 7 }}>選擇時段 — {selectedDate.replace(/-/g, "/")}</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 6 }}>
                {allTimeSlots.map((t) => {
                  const off = isOffSlot(selectedDate, t);
                  const picked = selectedTime === t;
                  return (
                    <div key={t} onClick={() => { if (!off) setSelectedTime(t); }} style={{ background: off ? (dark ? "#222" : "#F1EFE8") : picked ? "#534AB7" : cardBg, border: `0.5px solid ${picked ? "#534AB7" : cardBorder}`, borderRadius: 8, padding: "7px 0", textAlign: "center", fontSize: 12, color: off ? (dark ? "#444" : "#C8C6BE") : picked ? "#fff" : textMain, cursor: off ? "default" : "pointer" }}>
                      {t}
                      {off && <div style={{ fontSize: 8, color: dark ? "#555" : "#C8C6BE" }}>✕</div>}
                    </div>
                  );
                })}
              </div>
          ...
          {/* 摘要 */}
          <div style={{ background: summaryBg, border: "0.5px solid " + summaryBorder, borderRadius: 10, padding: "12px 14px", marginBottom: 12, fontSize: 12, color: summaryText, lineHeight: 2 }}>
            ...
            <div style={{ fontSize: 11, color: dark ? "#6B67A8" : "#888780", textAlign: "center" }}>實際價格由設計師現場確認</div>
          </div>
          ...
          <button
            ...
            style={{ width: "100%", background: (selectedServices.length > 0 && selectedDate && selectedTime) ? "#534AB7" : (dark ? "#333" : "#B4B2A9"), color: "#fff", border: "none", borderRadius: 10, padding: 12, fontSize: 14, fontWeight: 500, cursor: "pointer" }}
          >
            確認預約
          </button>
        </div>
