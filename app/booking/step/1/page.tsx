...
export default function Step1() {
  ...
  const noDesigner = { id: 0, name: "不指定", nickname: "依時間安排", initials: "✦", style: "依照時間由店家安排", ig: "", bg_color: "#F1EFE8", text_color: "#5F5E5A", work_hours: ["10:00","11:00","12:00","13:00","14:00","15:00","16:00","17:00"], avatar_url: "" };
  ...
  return (
    <div style={{ padding: "20px 16px 120px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        {/* 不指定設計師 */}
        <div
          onClick={() => setDesigner(noDesigner)}
          style={{
            background: selected?.id === 0 ? "#EEEDFE" : "#fff",
            border: "1.5px solid " + (selected?.id === 0 ? "#534AB7" : "#D3D1C7"),
            borderRadius: 14,
            padding: "16px 12px",
            textAlign: "center",
            cursor: "pointer",
        ...
        >
          <div style={{ width: 52, height: 52, borderRadius: "50%", background: "#F1EFE8", color: "#5F5E5A", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 600, margin: "0 auto 8px" }}>✦</div>
          <div style={{ fontSize: 14, fontWeight: 600, color: "#2C2C2A" }}>不指定設計師</div>
          <div style={{ fontSize: 11, color: "#888780", margin: "4px 0", lineHeight: 1.5 }}>依照我的時間，請店家幫我安排</div>
          {selected?.id === 0 && (
            <div style={{ marginTop: 8, background: "#534AB7", color: "#fff", borderRadius: 20, padding: "3px 12px", fontSize: 11, fontWeight: 600, display: "inline-block" }}>已選擇 ✓</div>
          )}
        </div>
        ...
        {designers.map((d) => {
          ...
          return (
            ...
            >
              ...
              {d.nickname ? <div style={{ fontSize: 11, color: "#7B6FD4", margin: "3px 0" }}>{d.nickname}</div> : null}
              <div style={{ fontSize: 11, color: "#888780", lineHeight: 1.5, margin: "4px 0 8px" }}>{d.style}</div>
              <a href={"https://www.instagram.com/" + d.ig} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} style={{ marginTop: 6, display: "inline-flex", alignItems: "center", gap: 4, textDecoration: "none" }}>
      ...
      <div style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 390, background: "#fff", borderTop: "0.5px solid #D3D1C7", padding: "12px 16px 24px" }}>
        <button
          onClick={() => { if (selected !== null) router.push("/booking/step/2"); }}
          disabled={selected === null}
          style={{
            width: "100%", padding: "13px 0",
            background: selected !== null ? "#534AB7" : "#D3D1C7",
            color: "#fff", border: "none", borderRadius: 12,
            fontSize: 15, fontWeight: 600,
            cursor: selected !== null ? "pointer" : "default",
          }}
        >
          {selected !== null ? "下一步：選擇服務" : "請先選擇設計師"}
        </button>
      </div>
    </div>
  );
